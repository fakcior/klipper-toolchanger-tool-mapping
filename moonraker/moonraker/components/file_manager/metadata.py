...
class BaseSlicer(object):
    ...
    def parse_filament_used(self) -> Optional[List[float]]:
        return None
...
class PrusaSlicer(BaseSlicer):
    ...
    def parse_total_toolchanges(self) -> Optional[float]:
        totalToolchanges = regex_find_int(r"; total filament change = (%D)", self.footer_data)
        if totalToolchanges is not None:
            return totalToolchanges
        else:
            return regex_find_int(r"; total toolchanges = (%D)", self.footer_data)
    ...
    def parse_filament_colors(self) -> Optional[str]:
        return regex_find_string(r'; filament_colour = (%S)', self.footer_data)

    def parse_filament_used_mm(self) -> Optional[str]:
        return regex_find_string(r'; filament used \[mm\] = (%S)', self.footer_data).replace(',', ';')
